...
### D-059 — Effect chips become image assets with per-token emoji fallback
**Date:** 2026-09-22 · **Context:** ballot polish, professional art.
**Decision:** effect icons move from emoji text to uploaded image assets. `EffectIcons` (client module) maps each Effects token to an asset id and falls back to the previous emoji per token until the id is filled in; `VoteUI` builds chip slots kind-aware (ImageLabel vs TextLabel) and rebuilds a slot only when a token's kind flips. Asset ids enter the repo only after a name-matched harvest (`tools/harvest_icon_ids.lua`), so paste order cannot mis-assign art. Icons are generated flat-line PNGs (`tools/generate_effect_icons.py`), uploaded once by hand, verified via `EffectIcons.countAssets()`.
**Consequence:** the ballot can ship before any upload exists (identical look via fallback), gains real icons token-by-token with zero code changes, and new effect tokens render no chip rather than a wrong one.

### D-060 — Effect chips get hover tooltips driven by a shared HELP table
**Date:** 2026-09-22 · **Context:** ballot polish, teach the mechanics vocabulary.
**Decision:** one shared tooltip frame per ScreenGui (sibling-level ZIndex, AutomaticSize-Y so wrap sizing is native) shown on chip MouseEnter. Copy lives in `EffectIcons.HELP` — one plain-language line per token, written for a player mid-vote; `helpFor` resolves aliases; unknown tokens show no tooltip. Wiring is at chip creation (pool slots persist across the replication tick, so per-render connects would stack handlers); the current token rides on the chip as an attribute, so a reused slot always describes what it is showing now. The tooltip never overlaps its chip (anchored above, flips below near screen top), so hover cannot flicker.
**Consequence:** the chip row now teaches itself — icon at a glance, sentence on hover. Verification note: the MCP bridge cannot synthesize mouse input (VIM lacks RobloxScript capability), so the hover path was verified structurally (frame built, attributes set, connections in the creation path) and by eye; VirtualInputManager is a Studio-only channel worth revisiting if the bridge ever gains script security.

### D-061 — Lexicon sync is now tested, not assumed
**Date:** 2026-09-22 · **Context:** finishing the icon/tooltip work.
Two silent failures were possible: a modifier declaring an `Effects` token with no art (chip renders nothing) or with no tooltip (hover shows nothing). A live audit also caught a phantom `PistolOnly` token — invented during icon generation and never declared by any modifier — sitting in the upload list, which would have had the user upload a dead icon.
**Decision:** `tests/effect_help_check.py` diffs the modifier registry's declared `Effects` tokens against `EffectIcons` EMOJI/ASSET/HELP and fails loudly on any gap. The phantom token was removed from ORDER, ASSET, GLYPH_OF and the icon set (the pistol glyph stays in the generator, commented, reserved for a future ranged token). `Example` is intentionally emoji-only.
**Consequence:** adding a modifier with a new effect token now fails a check until art and copy exist — the ballot can never ship a chip that renders or explains nothing.

### D-062 — A crate's look follows the weapon inside it, and the geometry constants follow the crate
**Date:** 2026-09-23 · **Context:** the first hand-made prop arrives — a pistol case modelled in Blender by an external agent — and the answer to "wire it in" is "only for weapons that are like pistols".
**Decision:** crate appearance moves into `CrateVisuals` (`ReplicatedStorage/Shared/Weapons`), switched by a new `Class` field on every weapon profile — `Sidearm` / `Primary` / `Melee`. A class with no entry gets `Default`, so a new weapon needs no crate code at all. A `Sidearm` crate is a `Part` (6.493 × 3.000 × 4.316) carrying a `SpecialMesh`; everything else keeps the 3×3×3 lime `Part`. Two numbers stop being constants in the same change: `LootService` derives its spawn offset from the visual's height (`size.Y/2 + UndersideLift`, because loot points are authored at `surfaceTop − 1`), and `ArenaValidator` derives its clearance box from `CrateVisuals.largestSize()`.
**Why the constants had to go:** crate size now varies, and both old numbers were silently coupled to the literal `3×3×3`. A hardcoded `+2.5` floats a shorter crate and buries a taller one; a hardcoded 3-stud clearance box measures a crate that no longer spawns. Neither is visible in code review, and both are the failure this validator exists to catch. The derived spawn offset computes to **2.50 — identical to the constant it replaced** — and the clearance half-diagonal goes 2.60 → 4.18 studs.
**The trap that cost three rounds.** The first implementation built a `MeshPart` and assigned `MeshId`. That write is **refused in Studio**: `The current thread cannot write 'MeshId' (lacking capability NotAccessible)` — from the round loop exactly as from a probe. Because `LootService.SpawnAll` runs inline in the round, the throw took the round with it: `[MR] WARN Round 1 aborted: ... MeshId`, three rounds running. `SpecialMesh.MeshId`/`TextureId` are **not** gated, and cloning an existing `MeshPart` preserves its `MeshId` (both verified live). A `Part` also collides as its box — the collision a crate wants — so `CollisionFidelity` stops being a crate concern entirely.
**Second decision, from the same failure:** `buildCrate` is `pcall`-guarded and falls back to the default block, and the spawn offset is measured off the body that was actually built. A crate's *appearance* must never be able to break a round, and a fallback must still land exactly on the floor.
**Evidence:** `tests/crate_visual_check.py` fails when a weapon declares no `Class`, on a typo'd class, on a `ByClass` entry no weapon uses, on a missing `MeshId`/`TextureId`/`NaturalSize`, and when `Size` is not a clean multiple of `NaturalSize` — a non-uniform stretch distorts a boxy mesh's brackets and seams all at once, which reads as a bad model rather than bad numbers. It prints the largest crate and the clearance that implies, so a resize shows up in the check output instead of only in an arena. The checker was proved against **six seeded breakages**, not assumed. The Foundry's worst loot clearance is 47.00 studs against 10–12 declared, so the larger case cannot spawn inside cover anywhere in the current arena.
**Proven in a running game, not just structurally:** after the fix, one full Studio match ran with **zero** round aborts, and a real round spawned 2 Pistol cases (6.493 × 3.000 × 4.316, `SpecialMesh` on mesh `rbxassetid://74022968545497`, uniform scale 3.649) beside 3 blocks (Sword, Sawn-Off, Rifle) — each crate's underside exactly 1.00 stud above its loot pad.
**Also settled:** the mesh ids now live in a committed file rather than only in `assets/PROVENANCE.md`, which is what makes them durable. That register's licence row for this asset is **still unread**, and the case is now shipping — so it moved from "someday" to "before the game is published".
**Not settled:** the case carries its own olive texture instead of the lime accent that makes crates the arena's one high-contrast object, so sidearm crates are now the quiet ones — deliberate for this round, and `CrateVisuals.ByClass.Sidearm.Color` is the tint knob. The sawn-off is classed `Primary`, so it ships in the block rather than the case; that is a one-word change if it reads wrong in play.
